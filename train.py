...
class PacmanPPOTrainer:
    def __init__(
        ...
        # Optimizers
        self.pacman_optimizer = optim.Adam(
            self.pacman_agent.parameters(), lr=learning_rate
        )
    ...
    def collect_trajectories(
        ...
        for _ in range(num_steps):
            ...
            with torch.no_grad():
                # Pacman action
                pacman_action, pacman_log_prob, _, pacman_value = (
                    self.pacman_agent.get_action_and_value(obs_tensor[0])
                )
                ...
                for i, ghost_agent in enumerate(self.ghost_agents):
                    action, log_prob, _, value = ghost_agent.get_action_and_value(
                        obs_tensor[i + 1]
                    )
            ...
            # Render if requested
            should_render = (
                self.render and self.render_episode_count % self.render_freq == 0
            )
            if should_render:
                self.env.render()
            ...
            for i, (action, log_prob, value) in enumerate(
                zip(ghost_actions, ghost_log_probs, ghost_values)
            ):
    ...
    def create_training_layout(self) -> Layout:
        ...
        layout["main"].split_row(
            Layout(name="stats", ratio=1), Layout(name="losses", ratio=1)
        )

        return layout

    def create_stats_table(
        self, timesteps: int, update_count: int, elapsed_time: float, fps: float
    ) -> Table:
        ...
        if recent_rewards:
            avg_rewards = [
                np.mean([r[i] for r in recent_rewards]) for i in range(self.num_agents)
            ]
            avg_scores = np.mean(self.pacman_scores[-20:]) if self.pacman_scores else 0
            avg_pellets = (
                np.mean(self.pellets_collected[-20:]) if self.pellets_collected else 0
            )
            ...
            table.add_row(
                "Ghost Catches", "-", f"{avg_catches:.1f}", f"{avg_catches:.1f}"
            )
            table.add_row(
                "Episodes Played",
                str(len(self.episode_rewards)),
                str(len(self.episode_rewards)),
                str(len(self.episode_rewards)),
            )
        ...
        table.add_row(
            "Updates", str(update_count), str(update_count), str(update_count)
        )
        table.add_row(
            "Training Time",
            f"{elapsed_time:.0f}s",
    ...
    def create_losses_table(self) -> Table:
        ...
        if all_have_losses:
            ...
            table.add_row(
                "Critic Loss",
                f"{recent_critic[0]:.4f}",
                f"{recent_critic[1]:.4f}",
                f"{recent_critic[2]:.4f}",
            )
            table.add_row(
                "Entropy Loss",
                f"{recent_entropy[0]:.4f}",
                f"{recent_entropy[1]:.4f}",
                f"{recent_entropy[2]:.4f}",
            )

            if all(len(values) > 0 for values in self.value_estimates):
                recent_values = [
                    np.mean(values[-5:]) for values in self.value_estimates
                ]
                recent_ratios = [np.mean(ratios[-5:]) for ratios in self.policy_ratios]
                recent_explained = [
                    np.mean(var[-5:]) for var in self.explained_variance
                ]

                table.add_row("", "", "", "")
                table.add_row(
                    "Value Estimate",
                    f"{recent_values[0]:.2f}",
                    f"{recent_values[1]:.2f}",
                    f"{recent_values[2]:.2f}",
                )
                table.add_row(
                    "Policy Ratio",
                    f"{recent_ratios[0]:.3f}",
                    f"{recent_ratios[1]:.3f}",
                    f"{recent_ratios[2]:.3f}",
                )
                table.add_row(
                    "Explained Var",
                    f"{recent_explained[0]:.3f}",
                    f"{recent_explained[1]:.3f}",
                    f"{recent_explained[2]:.3f}",
                )

        return table

    def create_progress_bar_text(
        self, timesteps: int, total_timesteps: int, elapsed_time: float
    ) -> str:
    ...
    def train(
        ...
        # Header
        header_text = Text(
            "🟡 PPO Pacman Training - Pacman vs Ghosts", style="bold magenta"
        )
        ...
        layout["stats"].update(
            Panel(
                Text("Starting training...", style="yellow"),
                title="🟡 Training Statistics",
                box=box.ROUNDED,
            )
        )
        layout["losses"].update(
            Panel(
                Text("Collecting initial data...", style="yellow"),
        ...
        with Live(layout, console=self.console, refresh_per_second=2) as live:
            while timesteps < total_timesteps:
                ...
                # Update all agents
                agents_and_optimizers = [
                    (self.pacman_agent, self.pacman_optimizer)
                ] + list(zip(self.ghost_agents, self.ghost_optimizers))

                for i, (agent, optimizer) in enumerate(agents_and_optimizers):
                    stats = self.update_agent(
                        agent, optimizer, trajectories[f"agent_{i}"]
                    )
                ...
                if update_count % 2 == 0 or not self.render:
                    ...
                    layout["stats"].update(
                        Panel(
                            self.create_stats_table(
                                timesteps, update_count, elapsed_time, fps
                            ),
                            box=box.ROUNDED,
                        )
                    )
                    ...
                    layout["losses"].update(
                        Panel(self.create_losses_table(), box=box.ROUNDED)
                    )

                    progress_bar_text = self.create_progress_bar_text(
                        timesteps, total_timesteps, elapsed_time
                    )
                ...
                if timesteps % save_frequency == 0 and timesteps > resume_from:
                    ...
                    if not self.render:
                        save_msg = Text(
                            f"💾 Pacman models saved at timestep {timesteps:,}",
                            style="bold green",
                        )
                        layout["progress"].update(
                            Panel(save_msg, title="Save Status", box=box.ROUNDED)
                        )
                        live.refresh()
                        time.sleep(1)
        ...
        elapsed_time = time.time() - start_time
        self.console.print(
            f"\n🎉 Pacman training completed in {elapsed_time:.2f} seconds!",
            style="bold green",
        )
    ...
    def save_models(self, filename_prefix: str, timesteps_completed: int = 0) -> None:
        ...
        # Add ghost models
        for i, (agent, optimizer) in enumerate(
            zip(self.ghost_agents, self.ghost_optimizers)
        ):
...
def plot_pacman_results(
    episode_rewards: List[List[float]],
    pacman_scores: List[float],
    ...
    if episode_rewards and len(episode_rewards) > 0:
        ...
        if len(pacman_rewards) >= 10:
            ...
            ma_pacman = np.convolve(
                pacman_rewards, np.ones(window) / window, mode="valid"
            )
            ma_ghost1 = np.convolve(
                ghost1_rewards, np.ones(window) / window, mode="valid"
            )
            ma_ghost2 = np.convolve(
                ghost2_rewards, np.ones(window) / window, mode="valid"
            )

            ax4.plot(
                range(window - 1, len(pacman_rewards)),
                ma_pacman,
                label="Pacman (MA)",
                color="gold",
                linewidth=2,
            ...
            ax4.plot(
                range(window - 1, len(ghost1_rewards)),
                ma_ghost1,
                label="Ghost 1 (MA)",
                color="red",
                linewidth=2,
            ...
            ax4.plot(
                range(window - 1, len(ghost2_rewards)),
                ma_ghost2,
                label="Ghost 2 (MA)",
                color="purple",
                linewidth=2,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train PPO agents on Pacman environment"
    )
    parser.add_argument(
        "--show", action="store_true", help="Show training visualization"
    )
    parser.add_argument(
        "--render-freq", type=int, default=5, help="Render every N episodes"
    )
    parser.add_argument(
        "--timesteps", type=int, default=300000, help="Total training timesteps"
    )
    ...
    try:
        ...
        trainer.save_models(
            "interrupted_models",
            trainer.current_step if hasattr(trainer, "current_step") else 0,
        )
