...
def run_pacman_demo_episode(env: PacmanEnv, pacman_agent: PacmanPPOAgent, ghost_agents: List[PacmanPPOAgent], device: torch.device, max_steps: int = 2000, speed: float = 0.1) -> Tuple[List[float], int]:
    ...
    while step_count < max_steps:
        ...
        with torch.no_grad():
            ...
            ghost_actions = []
            for ghost_agent in ghost_agents:
                action, _, _, _ = ghost_agent.get_action_and_value(obs_tensor[len(ghost_actions) + 1])
        ...
        if step_count % 100 == 0:
            print(f"Step {step_count}: Pacman Score={env.pacman.score}, Pellets={env.pellets_collected}/{env.total_pellets}, Alive={env.pacman.alive}")
        
        if done or truncated:
            print(f"\n🎮 Episode finished after {step_count} steps!")
            print(f"Final rewards: Pacman: {episode_rewards[0]:.1f}, Ghost1: {episode_rewards[1]:.1f}, Ghost2: {episode_rewards[2]:.1f}")
...
def main() -> int:
    ...
    parser.add_argument('--model', type=str, default='final_models.pth',
                        help='Path to trained model file')
    parser.add_argument('--episodes', type=int, default=3,
                        help='Number of episodes to run')
    parser.add_argument('--maze-width', type=int, default=19,
                        help='Maze width')
    ...
    try:
        ...
        print(f"Avg rewards - Pacman: {total_rewards[0]/episodes_played:.1f}, Ghost1: {total_rewards[1]/episodes_played:.1f}, Ghost2: {total_rewards[2]/episodes_played:.1f}")
