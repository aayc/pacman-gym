...
def main() -> None:
    parser = argparse.ArgumentParser(description='Quick Pacman PPO training with optimized parameters')
    parser.add_argument('--show', action='store_true', help='Show visualization')
    parser.add_argument('--timesteps', type=int, default=150000, help='Training timesteps')
    ...
    try:
        ...
        if episode_rewards:
            recent_episodes = episode_rewards[-20:] if len(episode_rewards) >= 20 else episode_rewards
            recent_scores = pacman_scores[-20:] if len(pacman_scores) >= 20 else pacman_scores
            recent_pellets = pellets_collected[-20:] if len(pellets_collected) >= 20 else pellets_collected
            
            print("\n📊 Quick Analysis (Last 20 Episodes):")
            if recent_scores:
                print(f"Avg Pacman Score: {sum(recent_scores)/len(recent_scores):.1f}")
            if recent_pellets:
                print(f"Avg Pellets Collected: {sum(recent_pellets)/len(recent_pellets):.1f}")
